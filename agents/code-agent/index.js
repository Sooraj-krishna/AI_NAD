...
const FileContentSchema = zod_1.z.object({
    path: zod_1.z.string().min(1, "File path cannot be empty"),
    content: zod_1.z.string().min(1, "File content cannot be empty")
});
const FileArraySchema = zod_1.z.array(FileContentSchema);
class CodeAgent {
    constructor(aiService) {
        this.aiService = aiService;
    }
    async process(architecture) {
        try {
            logger_1.Logger.pipelineStep('code_generated', { architecture });
            // --- DECOMPOSED GENERATION ---
            // 1. Generate Models/Entities
            logger_1.Logger.info('🚀 Step 4a: Generating Models...');
            const models = await this.generateModels(architecture);
            // 2. Generate Services (referencing Models)
            logger_1.Logger.info('🚀 Step 4b: Generating Services...');
            const services = await this.generateServices(architecture, models);
            // 3. Generate Controllers (referencing Services)
            logger_1.Logger.info('🚀 Step 4c: Generating Controllers...');
            const controllers = await this.generateControllers(architecture, services);
            // 4. Generate Routes (referencing Controllers)
            logger_1.Logger.info('🚀 Step 4d: Generating Routes...');
            const routes = await this.generateRoutes(architecture, controllers);
            // 5. Generate Frontend (referencing API)
            logger_1.Logger.info('🚀 Step 4e: Generating Frontend...');
            const frontendFiles = await this.generateFrontend(architecture);
            const backendFiles = [...models, ...services, ...controllers, ...routes];
            logger_1.Logger.info(`✅ Decomposed code generation complete: ${backendFiles.length} backend files, ${frontendFiles.length} frontend files`);
            return {
                success: true,
            ...
            logger_1.Logger.error('Code Agent error', error);
            return {
                success: false,
     ...
     */
    async repair(errors, currentCode, architecture, userPrompt) {
        try {
            logger_1.Logger.info(`🔧 Repairing code based on ${errors.length} validation errors...`);
...
Please fix the errors and return the corrected files in a JSON array.`;
            ...
            logger_1.Logger.error('Code Agent repair error', error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error'
            };
    ...
    async generateServices(architecture, models) {
    ...
    async generateControllers(architecture, services) {
    ...
    async generateRoutes(architecture, controllers) {
    ...
    async generateFrontend(architecture) {
    ...
    parseFileArray(text, label) {
        try {
            ...
            return validatedFiles.map(f => {
                ...
                return f;
            }).filter((f) => Boolean(f.path && f.content));
        }
        catch (err) {
            logger_1.Logger.warn(`⚠️ Could not parse ${label} file array. This indicates a hallucination in the AI response format. Returning empty array to force pipeline failure or repair.`, err);
